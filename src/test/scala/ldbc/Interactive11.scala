...
class Interactive11 extends Simulation {
  ...
  val csvFeeder: Iterator[Map[String, Any]] = separatedValues("snb-iq11-params.csv", '|').random()
  ...
  def request: HttpRequestBuilder = {
    http("LDBC SNB IQ 11")
      ...
      .body(StringBody("""ldbc_snb_iq11("#{personId}","#{countryName}",#{workFromYear})""".stripMargin))
      //.asJson
      .check(status.is(200))
  }

  val scn: ScenarioBuilder = scenario("rage.ldbc.snb.iq11")
