...
class Interactive07 extends Simulation {
  ...
  val csvFeeder: Iterator[Map[String, Any]] = separatedValues("snb-iq07-params.csv", '|').random()
  ...
  def request: HttpRequestBuilder = {
    http("LDBC SNB IQ 07")
      ...
      .body(StringBody("""ldbc_snb_iq07("#{personId}")""".stripMargin))
      //.asJson
      .check(status.is(200))
  }

  val scn: ScenarioBuilder = scenario("rage.ldbc.snb.iq07")
