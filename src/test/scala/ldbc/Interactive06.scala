...
class Interactive06 extends Simulation {
  ...
  val csvFeeder: Iterator[Map[String, Any]] = separatedValues("snb-iq06-params.csv", '|').random()
  ...
  def request: HttpRequestBuilder = {
    http("LDBC SNB IQ 06")
      ...
      .body(StringBody("""ldbc_snb_iq06("#{personId}","#{tagName}")""".stripMargin))
      //.asJson
      .check(status.is(200))
  }

  val scn: ScenarioBuilder = scenario("rage.ldbc.snb.iq06")
