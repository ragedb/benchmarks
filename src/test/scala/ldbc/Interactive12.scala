...
class Interactive12 extends Simulation {
  ...
  val csvFeeder: Iterator[Map[String, Any]] = separatedValues("snb-iq12-params.csv", '|').random()
  ...
  def request: HttpRequestBuilder = {
    http("LDBC SNB IQ 12")
      ...
      .body(StringBody("""ldbc_snb_iq12("#{personId}","#{tagClassName}")""".stripMargin))
      //.asJson
      .check(status.is(200))
  }

  val scn: ScenarioBuilder = scenario("rage.ldbc.snb.iq12")
