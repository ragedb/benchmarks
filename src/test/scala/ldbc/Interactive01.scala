...
class Interactive01  extends Simulation {
  ...
  val csvFeeder: Iterator[Map[String, Any]] = csv("snb-iq01-params.csv").random()
  ...
  def request: HttpRequestBuilder = {
    http("LDBC SNB IQ 01")
      ...
      .body(StringBody("""ldbc_snb_iq01("#{personId}","#{firstName}")""".stripMargin))
      //.asJson
      .check(status.is(200))
  }

  val scn: ScenarioBuilder = scenario("rage.ldbc.snb.iq01")
