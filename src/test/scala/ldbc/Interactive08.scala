...
class Interactive08 extends Simulation {
  ...
  val csvFeeder: Iterator[Map[String, Any]] = separatedValues("snb-iq08-params.csv", '|').random()
  ...
  def request: HttpRequestBuilder = {
    http("LDBC SNB IQ 08")
      ...
      .body(StringBody("""ldbc_snb_iq08("#{personId}")""".stripMargin))
      //.asJson
      .check(status.is(200))
  }

  val scn: ScenarioBuilder = scenario("rage.ldbc.snb.iq08")
