...
class CreateNodeFakeProperties extends Simulation {
  ...
  val fakeFeeder: Iterator[Map[String, String]] = Iterator.continually(
    Map(
  ...
  def createPostRequest: HttpRequestBuilder = {
    http("CreateNodeFakeProperties")
      .post("/db/" + params.rageDB + "/nodes/Address/${uuid}")
      .body(StringBody(   """{ "street_address": "${street_address}", "city": "${city}","state": "${state}","zip_code": "${zip_code}" }"""))
      .asJson
      .check(status.is(201))
  }
